...
export default function QuizCard(props) {
    ...
    const safeModules = Array.isArray(modules)
          ? modules.map(m => (typeof m === "string" ? m : (m?.module_name ?? m?.name ?? ""))).filter(Boolean)
          : [];
    const safeTags = Array.isArray(tags)
      ? tags.map(t => (typeof t === "string" ? t : (t?.tag_name ?? t?.name ?? ""))).filter(Boolean)
      : [];
        ...
        <Section>
           ...
           <ModulesRow>
             {safeModules.slice(0, 3).map((mod, i) => (
              <Tag key={i} variant="primary">{mod}</Tag>
             ))}
             {safeModules.length > 3 && (
               <Tag variant="primary">+{safeModules.length - 3}</Tag>
             )}
           </ModulesRow>
         )}
            <TagsRow>
